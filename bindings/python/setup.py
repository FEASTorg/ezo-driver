...
core_sources = sorted(relpath(path) for path in (CSRC_DIR / "src").glob("*.c"))
linux_sources = sorted(relpath(path) for path in (CSRC_DIR / "platform" / "linux").glob("*.c"))

ext_modules = [
    Extension(
        "ezo_driver_py._core",
        ["src/ezo_driver_py/_core.pyx", *core_sources, *linux_sources],
        include_dirs=[relpath(INCLUDE_DIR)],
