/*
Purpose: simple Arduino UART HUM read with explicit response-code bootstrap and output-config ownership.
Defaults: 9600 baud on the shared UART helper stream.
Assumptions: the connected device is a HUM circuit reachable over UART.
...
static void request_output_config() {
  ...
  EZO_ARDUINO_CHECK_OK("send_output_query", ezo_hum_send_output_query_uart(&device, &hint));
  ezo_arduino_wait_hint(&hint);
  EZO_ARDUINO_CHECK_OK("read_output_query", ezo_hum_read_output_config_uart(&device, &output_config));
}

static void request_reading() {
  ezo_timing_hint_t hint;

  EZO_ARDUINO_CHECK_OK("send_read", ezo_hum_send_read_uart(&device, &hint));
  ezo_arduino_wait_hint(&hint);
}
...
void loop() {
  ...
  if (read_requested == 0U) {
    ...
    ezo_arduino_uart_bootstrap_response_codes(&device, EZO_PRODUCT_HUM);
  ...
  EZO_ARDUINO_CHECK_OK("read_response",
                       ezo_hum_read_response_uart(&device, output_config.enabled_mask, &reading));
